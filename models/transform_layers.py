...
class TransformList(nn.Module):
    def __init__(self, transform: List[nn.Module], transform_max_range: List[int], skip_idx = None):
        super(TransformList, self).__init__()

        assert len(transform) == len(transform_max_range)

        self.idx_cum = torch.cumsum(torch.Tensor(transform_max_range), dim=0).type(dtype=torch.LongTensor)
        self.transform = nn.ModuleList(transform)

        if skip_idx is None:
            skip_idx = [0 for _ in transform]
        assert len(transform) == len(skip_idx)
        self.skip_idx = skip_idx

    def forward(self, input, aug_index=None):
        if aug_index is None:
            aug_index = np.random.randint(self.idx_cum[-1]) 
        
        assert aug_index < self.idx_cum[-1]

        t_idx = (aug_index < self.idx_cum).nonzero(as_tuple=False)[0].type(dtype=torch.LongTensor)
        if t_idx > 0:
            aug_index = (aug_index - self.idx_cum[t_idx-1]).type(dtype=torch.LongTensor).item()

        output = self.transform[t_idx](input, aug_index + self.skip_idx[t_idx])
...
class Identity(nn.Module):    
    ...
    def forward(self, input: Tensor, aug_index=None) -> Tensor:
        return input
...
class MixUp(nn.Module):
    ...
    def forward(self, img1, img2):
        ...
        return img1 * (1 - self.alpha) + img2 * self.alpha

class BatchMix(nn.Module):
    def __init__(self, alpha: float = 0.5):
        super(BatchMix, self).__init__()
        self.mix = MixUp(alpha)

    def forward(self, input, aug_index=1):
        B, _, H, W = input.size()
        assert B > 1
        ...
        if aug_index is None:
            aug_index = np.random.randint(B)

        shift_idx = ((torch.arange(B) + aug_index + 1) % B).to(_device)

        output = self.mix(input, input[shift_idx])

        return output

class ShiftMix(nn.Module):
    def __init__(self, shift_transform, shift_max = 4, alpha: float = 0.5):
        super(ShiftMix, self).__init__()
        self.mix = MixUp(alpha)
        self.shift = shift_transform
        self.shift_max = shift_max

    def forward(self, input, aug_index=0):   
        _device = input.device                

        if aug_index is None:
            aug_index = np.random.randint(self.shift_max)

        # shift index 0 is usually identity, skip it
        shift = self.shift(input, aug_index + 1)

        output = self.mix(input, shift)

        return output

class CropMix(nn.Module):
    def __init__(self, alpha: float = 0.5, size=None, scale=(0.08, 1.0), ratio=(3. / 4., 4. / 3.)):
        super(CropMix, self).__init__()
        self.mix = MixUp(alpha)
        self.crop = RandomResizedCropLayer(size, scale, ratio)

    def forward(self, input, aug_index=None):
        crop_1 = self.crop(input)
        crop_2 = self.crop(input)

        output = self.mix(crop_1, crop_2)

        return output

class AugMix(nn.Module):
    def __init__(self, aug: nn.Module, alpha: float = 0.5):
    ...
    def forward(self, input, aug_index=None):
        aug_1 = self.aug(input)
        aug_2 = self.aug(input)

        output = self.mix(aug_1, aug_2)
...
class HorizontalFlipLayer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        N = inputs.size(0)
        _theta = self._eye.repeat(N, 1, 1)
        r_sign = torch.bernoulli(torch.ones(N, device=_device) * self.prob) * 2 - 1
        _theta[:, 0, 0] = r_sign
        grid = F.affine_grid(_theta, inputs.size(), **kwargs).to(_device)
