...
def eval_ood_tsne(P, model, id_loader, ood_loaders, ood_scores, train_loader=None, simclr_aug=None):
    ...
    fea_dim = feats_train_mean[0].shape[1]
    label_train = [P.target_class] * (train_count * P.K_shift) # M
    feats_train_np = [f.cpu().detach().numpy() for f in feats_train_mean] # list of (M, d)
    
    feats_train_all = [feats_train_np[k] for k in range(P.K_shift)]
    shift_label_train = [[k] * train_count for k in range(P.K_shift)]
    shift_label_train = np.reshape(shift_label_train, -1)
    ...
    feats_test_all = [feats_id_np[k] for k in range(P.K_shift)]
    label_test_all = list(label_id) * P.K_shift # need a new list here
    shift_label_test_all = [[[k] * len(feats_id_np[k]) for k in range(P.K_shift)]]
    for ood_class in P.ood_dataset:
        for k in range(P.K_shift):
            feats_test_all.append(feats_ood_np[ood_class][k])
        label_test_all += label_ood[ood_class] * P.K_shift

        s_label = [[k] * len(feats_ood_np[ood_class][k]) for k in range(P.K_shift)]
        shift_label_test_all.append(s_label)
    shift_label_test_all = np.reshape(shift_label_test_all, -1)

    feats_all_mean = np.concatenate(feats_train_all + feats_test_all)

    print("fitting t-SNE...")
    tsne = manifold.TSNE(n_components=2, random_state=0).fit_transform(feats_all_mean)
    train_tsne, test_tsne = np.split(tsne, [train_count * P.K_shift])

    print("plotting t-SNE...")
    tsne_fig = visualize_tsne(x_train=train_tsne, y_train=label_train, s_train=shift_label_train, 
                              x_test=test_tsne, y_test=label_test_all, s_test=shift_label_test_all, 
                              target_class=P.target_class, label_map=label_map)

    print("saving t-SNE...")
    fig_path = prefix + "_tsne" + "_f_" + feature_layer + "_C_" + str(P.target_class) + ".png"
...
train_colors = ['black', 'red', 'green', 'blue', 'cyan', 'magenta', 'yellow']
id_colors = ['gray', 'pink', 'lime', 'mediumblue', 'darkcyan', 'gold']
ood_colors = ['brown', 'orange', 'lightseagreen', 'royalblue', 'purple', 'crimson', 'olive']
def visualize_tsne(x_train, y_train, s_train, x_test, y_test, s_test, target_class, label_map: Dict = None) -> Figure:
    """
    x: data
    y: label
    s: transformation label
    target_class:
    label_map: mapping label from string to integer (or something else)
    """
    ...
    for i in range(x_train_norm.shape[0]):
        ...
        color = train_colors[s_train[i]]

        plt.text(x_train_norm[i, 0], x_train_norm[i, 1], y_str, color=color, 
    ...
    for i in range(x_test_norm.shape[0]):
        ...
        if y_test[i] == target_class:
            color = id_colors[s_test[i]]
        else:
            color = ood_colors[s_test[i]]
