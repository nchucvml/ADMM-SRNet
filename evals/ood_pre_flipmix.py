...
hflip_1 = TL.HorizontalFlipLayer(1.0).to(device)
...
def eval_ood_detection(P, model, id_loader, ood_loaders, ood_scores, train_loader=None, simclr_aug=None):
    ...
    feature_prefix = os.path.join(base_path, f'feats_{prefix}')
    ...
    feats_train = get_features(P, f'{P.dataset}_train', model, train_loader, prefix=feature_prefix, **kwargs)  # (M, T, d)
    ...
    feats_id = get_features(P, P.dataset, model, id_loader, prefix=feature_prefix, **kwargs)  # (N, T, d)
    feats_ood = dict()
    for ood, ood_loader in ood_loaders.items():
        if ood == 'interp':
            feats_ood[ood] = get_features(P, ood, model, id_loader, interp=True, prefix=feature_prefix, **kwargs)
        else:
            feats_ood[ood] = get_features(P, ood, model, ood_loader, prefix=feature_prefix, **kwargs)
...
def get_scores(P, feats_dict, ood_score):
    ...
    # [batch] [ensemble] [rotation] [feature] 
    ...
    for f_sim, f_shi in zip(feats_sim, feats_shi):
        ...
        score = 0        
        # [rotation] [feature] 
        for shi in range(len(P.weight_sim)):
            # [feature] 
...
def _get_features(P, model, loader, interp=False, imagenet=False, simclr_aug=None,
    ...
    for i, (x, _) in enumerate(loader):
        ...
        for seed in range(sample_num):
            ...
            if P.K_shift > 1 and 'bmix' in P.shift_trans_type:
                x_temp = [P.shift_trans(hflip(x), k) for k in range(P.K_shift - 1)]
                x_mix = hflip_1(x) * 0.5 + x * 0.5
                x_temp.append(x_mix)
