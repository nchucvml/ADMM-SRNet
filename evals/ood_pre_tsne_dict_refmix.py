...
def eval_ood_tsne(P, model, id_loader, ood_loaders, ood_scores, train_loader=None, simclr_aug=None):
    ...
    # model
    sr_net = model.sr_nets
    sr_dict = sr_net.get_dictionary().detach().cpu() # [k_shift, f_dim, dict_sz]

    model = model.feature_net
    ...
    for k in range(P.K_shift):
        f = feats_train_mean[k]
        _dict = sr_dict[k]

        eps = 1e-8
        _dict *= (f.norm(dim=1).mean() / (_dict.norm(dim=0).mean() + eps))
    ...
    feats_id_mean = [] # list of (N, d)   
    for f in feats_id[feature_layer].chunk(P.K_shift, dim=1):
        f = f.detach().mean(dim=1)
    ...
    for ood_idx, (ood, ood_loader) in enumerate(ood_loaders.items()):
        ...
        if ood == 'interp':
            feats_ood[ood] = get_features(P, ood, model, id_loader, interp=True, prefix=prefix, ref_batch=ref_batch, **kwargs)
        else:
            feats_ood[ood] = get_features(P, ood, model, ood_loader, prefix=prefix, ref_batch=ref_batch, **kwargs)
        
        feats_ood_mean[ood_class] = [] # list of (N, d)   
        for f in feats_ood[ood][feature_layer].chunk(P.K_shift, dim=1):
            f = f.detach().mean(dim=1)
            ...
            feats_ood_mean[ood_class].append(f)
    ...
    feats_all_mean = [[sr_dict[k].t().numpy(), feats_train_np[k], feats_id_np[k]] for k in range(P.K_shift)]
    ...
    for k in range(P.K_shift):
        ...
        dict_tsne, train_tsne, test_tsne = np.split(tsne, [P.sr_dict_size, P.sr_dict_size + train_count])

        print("plotting t-SNE...")
        tsne_fig = visualize_tsne(x_train=train_tsne, y_train=label_train, 
                                  x_test=test_tsne, y_test=label_test_all, 
                                  x_dict=dict_tsne, 
                                  target_class=P.target_class, label_map=label_map)

        print("saving t-SNE...")
        fig_path = prefix + "_tsne_dict" + "_f_" + feature_layer + "_C_" + str(P.target_class) + "_S_" + str(k) + ".png"
...
def visualize_tsne(x_train, y_train, x_test, y_test, x_dict, target_class, label_map: Dict = None) -> Figure:

    x_all = np.concatenate((x_dict, x_train, x_test))
    ...
    x_dict_norm = (x_dict - x_min) / (x_max - x_min)  #Normalize
    ...
    for i in range(x_dict_norm.shape[0]):
        y_str = "D"

        plt.text(x_dict_norm[i, 0], x_dict_norm[i, 1], y_str, color='g', 
