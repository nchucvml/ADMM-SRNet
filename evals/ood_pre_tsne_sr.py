...
def eval_ood_tsne(P, model, id_loader, ood_loaders, ood_scores, train_loader=None, simclr_aug=None):
    ...
    print('Compute sr dict')
    # TODO: compute this in an independent program
    sr_dict = []
    for f in feats_train_mean:        
        _dict = get_dictionary(f, dict_size=P.sr_dict_size, sr_lambda=P.sr_lambda)
        sr_dict.append(_dict)
    ...
    feats_all_mean = [[sr_dict[k].T, feats_train_np[k], feats_id_np[k]] for k in range(P.K_shift)]
    ...
    for k in range(P.K_shift):
        ...
        print("saving t-SNE...")
        fig_path = prefix + "_tsne_sr" + "_f_" + feature_layer + "_C_" + str(P.target_class) + "_S_" + str(k) + ".png"
