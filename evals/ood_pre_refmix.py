...
def eval_ood_detection(P, model, id_loader, ood_loaders, ood_scores, train_loader=None, simclr_aug=None, simclr_aug_no_crop=None):
    ...
    feats_id = get_features(P, P.dataset, model, id_loader, prefix=feature_prefix, ref_batch=ref_batch, **kwargs)  # (N, T, d)
    ...
    for ood, ood_loader in ood_loaders.items():
        if ood == 'interp':
            feats = get_features(P, ood, model, id_loader, interp=True, prefix=feature_prefix, ref_batch=ref_batch, **kwargs)
        else:
            feats = get_features(P, ood, model, ood_loader, prefix=feature_prefix, ref_batch=ref_batch, **kwargs)
    ...
    for i, (x, _) in enumerate(loader):
        ...
        for seed in range(sample_num):
            ...
            if P.K_shift > 1 and 'bmix' in P.shift_trans_type and ref_batch is not None:
                ...
                x_mix = hflip(x) * 0.5 + hflip(ref_batch[ref_i]) * 0.5
                x_temp.append(x_mix)
