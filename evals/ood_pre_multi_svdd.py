...
def eval_ood_detection(P, model, id_loader, ood_loaders, ood_scores, train_loader=None, simclr_aug=None):
    ...
    for f in feats_train[sr_feature_layer].chunk(P.K_shift, dim=1):
        ...
        _svdd =  get_svdd(axis)
        P.svdds.append(_svdd)
    time_end = time.perf_counter()
    print('axis size: ' + ' '.join(map(lambda x: str(len(x)), P.axis)))
    print('svdd model build time {}'.format((time_end - time_start)))
...
def get_scores(P, feats_dict, ood_score, feature_layer="simclr"):
    ...
    for shift_idx, f in enumerate(feats_sim.chunk(P.K_shift, dim=1)):
        ...
        dist = get_svdd_dist(feature, P.svdds[shift_idx])
...
def get_svdd(features):
    print("SVDD lerning...")

    features = features.cpu().numpy()
    fake_label = np.ones((features.shape[0], 1))
    ...
    parameters = {"positive penalty": 0.9,
              ...
              "option": {"display": 'on'}}
    _svdd = svdd.SVDD(parameters)

    # train
    _svdd.train(features, fake_label)
    
    return _svdd

def get_svdd_dist(features, svdd_model: svdd.SVDD):
    """
    Args:
        features: (np.ndarray) N x Dim
        svdd_model: 
    Returns:
        (np.ndarray) distance to sphere center?, N x 1
    """

    fake_label = np.ones((features.shape[0], 1))

    dist, _ = svdd_model.test(features, fake_label)
