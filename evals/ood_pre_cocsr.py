...
def eval_ood_detection(P, model, id_loader, ood_loaders, ood_scores, train_loader=None, simclr_aug=None, simclr_aug_no_crop=None):
    ...
    kwargs = {
        ...
        'simclr_aug_no_crop': simclr_aug_no_crop,
    ...
    for ood, ood_loader in ood_loaders.items():
        if ood == 'interp':
            feats = get_features(P, ood, model, id_loader, interp=True, prefix=prefix, **kwargs)
        else:
            feats = get_features(P, ood, model, ood_loader, prefix=prefix, **kwargs)
...
def get_features(P, data_name, model, loader, interp=False, prefix='',
                 simclr_aug=None, simclr_aug_no_crop=None, 
    ...
    if len(left) > 0:
        _feats_dict = _get_features(P, model, loader, interp, P.dataset == 'imagenet', 
                                    simclr_aug, simclr_aug_no_crop, 
                                    sample_num, layers=left)
...
def _get_features(P, model, loader, interp=False, imagenet=False, 
                  simclr_aug=None, simclr_aug_no_crop=None, 
                  sample_num=1, layers=('simclr', 'shift')):
    ...
    # batch_size = 1 image with multi samples for ImageNet or crop mix mode
    # don't need more random sampling 
    if imagenet is True:  
        sample_num = 1
    ...
    for i, (x, _) in enumerate(loader):
        ...
        for seed in range(sample_num):
            ...
            if P.shift_trans_type == 'rot_cropmix':
                ...
                x_t_rot = torch.cat([P.shift_trans(x_t, k) for k in range(P.K_shift-1)])
                x_t_cmix = P.shift_trans(x_t, P.K_shift-1)

                # transform
                x_t_rot = simclr_aug(x_t_rot)  
                if P.cropmix_crop_aug:
                    x_t_cmix = simclr_aug(x_t_cmix)
                else:
                    x_t_cmix = simclr_aug_no_crop(x_t_cmix) 

                x_t = torch.cat([x_t_rot, x_t_cmix], dim=0)  # KB
                ...
                # augmix
                x_t_amix = P.shift_trans(x_t, P.K_shift-1)

                # concat
                x_t = torch.cat([x_t_rot, x_t_amix], dim=0)  # KB

            else:
                x_t = torch.cat([P.shift_trans(x_t, k) for k in range(P.K_shift)])                
                x_t = simclr_aug(x_t)  # simclr augmentation
