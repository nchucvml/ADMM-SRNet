...
def eval_ood_detection(P, model, id_loader, ood_loaders, ood_scores, train_loader=None, simclr_aug=None, simclr_aug_no_crop=None):
    ...
    for ood, ood_loader in ood_loaders.items():
        if ood == 'interp':
            feats = get_features(P, ood, model, id_loader, interp=True, prefix=feature_prefix, **kwargs)
        else:
            feats = get_features(P, ood, model, ood_loader, prefix=feature_prefix, **kwargs)
