...
def eval_ood_detection(P, model, id_loader, ood_loaders, ood_scores, train_loader=None, simclr_aug=None):
    ...
    for f in feats_train[feature_layer].chunk(P.K_shift, dim=1):
        ...
        dict_ =  get_dictionary(axis, dict_size=P.sr_dict_size, sr_lambda=P.sr_lambda)
        P.dict.append(dict_)
    time_end = time.perf_counter()
    print('axis size: ' + ' '.join(map(lambda x: str(len(x)), P.axis)))
    print('dictionary build time {}'.format((time_end - time_start)))
    
    # SR of train features
    time_start = time.perf_counter()
    for shift_idx, f in enumerate(feats_train[feature_layer].chunk(P.K_shift, dim=1)):
        feature = f.mean(dim=1)  # (M, d)
        feature = normalize(feature, dim=1)

        sr_coef = get_sr_coef(feature, P.dict[shift_idx], P.sr_lambda)
    time_end = time.perf_counter()
    print('train feature sr time {}'.format((time_end - time_start)))
...
def get_scores(P, feats_dict, ood_score, feature_layer="simclr"):
    ...
    for f_sim in tqdm(feats_sim):
        f_sim = [f.mean(dim=0) for f in f_sim.chunk(P.K_shift)]  # list of (1, d)        
        score = 0
