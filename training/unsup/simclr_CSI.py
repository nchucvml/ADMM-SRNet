    ...
    for n, (images, labels) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            if P.shift_trans_type == 'rot_cropmix':
                ...
                images_rot_1 = torch.cat([P.shift_trans(images1, k) for k in range(P.K_shift-1)])
                images_cmix_1 = P.shift_trans(images1, P.K_shift-1)

                images_rot_2 = torch.cat([P.shift_trans(images2, k) for k in range(P.K_shift-1)])
                images_cmix_2 = P.shift_trans(images2, P.K_shift-1)

                # transform
                images_rot_1 = simclr_aug(images_rot_1)  
                images_rot_2 = simclr_aug(images_rot_2)                   

                if P.cropmix_crop_aug:                    
                    images_cmix_1 = simclr_aug(images_cmix_1) 
                    images_cmix_2 = simclr_aug(images_cmix_2) 
                else:
                    images_cmix_1 = simclr_aug_no_crop(images_cmix_1) 
                    images_cmix_2 = simclr_aug_no_crop(images_cmix_2) 

                images_pair = torch.cat([images_rot_1, images_cmix_1, images_rot_2, images_cmix_2], dim=0)  # KB
                ...
                # augmix
                images_amix_1 = P.shift_trans(images1, P.K_shift-1)
                images_amix_2 = P.shift_trans(images2, P.K_shift-1)

                # concat
                images_pair = torch.cat([images_rot_1, images_amix_1, images_rot_2, images_amix_2], dim=0)  # KB
            ...
            shift_labels = torch.cat([torch.ones_like(labels) * k for k in range(P.K_shift)], 0)  # B -> KB
